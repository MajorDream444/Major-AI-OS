        ...
        <div className="cinematic-panel grid grid-cols-1 lg:grid-cols-[0.9fr_1.1fr] gap-0 items-stretch">
          <article className="relative z-10 p-6 md:p-8 lg:border-r border-brand-gold/15">
            <div className="relative z-10 flex items-center justify-between gap-4">
              <div>
                <p className="font-mono text-xs uppercase tracking-[0.2em] text-brand-gold">Next Knowledge Session</p>
                <h2 className="mt-5 font-serif text-4xl md:text-5xl leading-tight text-brand-white">
                  This is the room for the next step.
                </h2>
              </div>
            ...
            <div className="relative z-10 mt-8 divide-y divide-brand-gold/15 border-y border-brand-gold/15">
              <div className="grid grid-cols-1 sm:grid-cols-[0.55fr_1fr_0.45fr] gap-3 py-5">
                <p className="font-mono text-[0.68rem] uppercase tracking-[0.18em] text-brand-muted">{nextSession.bali.label}</p>
                <p className="text-2xl font-semibold text-brand-white">{nextSession.bali.date}</p>
                <p className="flex items-center gap-2 text-brand-gold"><Clock size={16} /> {nextSession.bali.time}</p>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-[0.55fr_1fr_0.45fr] gap-3 py-5">
                <p className="font-mono text-[0.68rem] uppercase tracking-[0.18em] text-brand-muted">{nextSession.newYork.label}</p>
                <p className="text-2xl font-semibold text-brand-white">{nextSession.newYork.date}</p>
                <p className="flex items-center gap-2 text-brand-gold"><Clock size={16} /> {nextSession.newYork.time}</p>
              </div>
            </div>
            <div className="relative z-10 mt-6 flex flex-wrap items-center gap-3 font-mono text-xs uppercase tracking-[0.16em] text-brand-muted">
              <span className="inline-flex items-center gap-2 text-brand-green"><Radio size={14} /> {nextSession.location}</span>
