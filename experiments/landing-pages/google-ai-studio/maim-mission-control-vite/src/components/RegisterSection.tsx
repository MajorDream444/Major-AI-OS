...
export default function RegisterSection() {
  ...
  return (
    <section id="register" className="bg-brand-black-light py-24 md:py-32 border-b border-brand-gold/10">
      <div className="max-w-3xl mx-auto px-6 md:px-12 text-center">
        <div className="w-16 h-px bg-brand-gold mx-auto mb-8" />
        <p className="text-xs uppercase tracking-[0.2em] text-brand-gold mb-6">Next session</p>
        <h2 className="font-serif text-4xl md:text-6xl text-brand-white leading-tight">Reserve your free seat</h2>
        ...
        <form onSubmit={handleSubmit} className="mt-10 border border-brand-gold/30 bg-brand-black/70 p-6 md:p-8 text-left space-y-5">
          ...
          <button disabled={status === 'sending'} className="w-full inline-flex items-center justify-center gap-2 bg-brand-gold text-brand-black px-8 py-4 text-xs font-bold uppercase tracking-[0.2em] hover:bg-brand-gold-muted transition disabled:opacity-60">
            {status === 'sending' ? 'Sending' : 'Register free'} <ArrowRight size={16} />
          </button>
          {status === 'success' && <p className="flex items-center gap-2 text-brand-green"><CheckCircle2 size={18} /> You are registered. Check your email for the confirmation.</p>}
          {status === 'error' && <p className="text-brand-orange">Something did not connect. Use the backup form link below.</p>}
        </form>
